...
Because the local client imports the implementation, place it behind a server-only module boundary. Browser code should build a separate client from the same contract with `fetchTransport()` or another remote transport. The [hybrid rendering guide](/docs/api/full-stack/hybrid-rendering) shows safe placements for full-stack frameworks.
