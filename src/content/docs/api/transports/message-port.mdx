...
```ts
const server = messagePortAdapter(serverPort, { channel: 'admin-api' }).mount(
  implementation
)
...
For ready-made host helpers and JSON-safe relays, see [desktop bridges](/docs/api/transports/desktop-bridges).
