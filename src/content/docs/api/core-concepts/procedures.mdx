...
If you plan on using `@hulla/api` only on client, you can skip this section. Otherwise I'd stongly recommend giving it a read
...
that's famework or library specfic should not be part of the procedure function declaration. Instead you should import the client-side logic of that library
only when necessary to avoiding importing it on server or parts of client where you don't need it.
