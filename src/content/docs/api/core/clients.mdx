...
description: 'Turn the shared contract into callable functions, send requests, handle declared responses, and add client middleware.'
---

`createClient(contractOrSelection, options)` returns a plain tree of endpoint functions, or a single function when given a route. Start with the shared contract from [contract authoring](/docs/api/core/contracts). Construction prepares the selected routes once; every call returns a promise. Middleware is supplied in the options.

```ts filename="src/api/client.ts" focus={5-7,9-12,14-16}
...
await client.users.rename(input, { signal: controller.signal })
```
...
state layer. The optional [TanStack Query and SWR integrations](/docs/api/integrations/query-libraries) provide keys and executable options without
...
```ts filename="src/api/client.ts" focus={5-8,10-12}
const session = { accessToken: 'example-token' }
...
```ts filename="src/api/clients.ts" focus={16-26}
import { clientMiddleware, createClient } from '@hulla/api/client'
...
const logRequests = clientMiddleware(contract, ({ next, route }) => {
  console.info(route.key.join('.'))
  return next()
})
const authenticate = clientMiddleware(contract, ({ next, request }) => {
...
export const users = createClient(contract.routes.users, {
  transport,
  middleware: [logRequests, authenticate],
})

// Optional application namespace.
export const client = { health, users }
```
...
For framework-specific client placement, server-only import guards, and native server-function patterns, see
[server and browser calls](/docs/api/full-stack/hybrid-rendering).
...
export async function renameUser(
  input: Parameters<typeof client.users.rename>[0]
) {
  const result = await client.users.rename(input)
...
[`@hulla/api-message-port` transport](/docs/api/transports/message-port). It preserves the same client call surface across a process or
...
Static `headers` objects are captured and normalized when the client is constructed. Supply a function that returns the current headers when values must change per call.
...
export const client: ClientFor<typeof contract> = createClient(contract, {
  transport,
})
```
