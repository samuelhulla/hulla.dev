...
For the recommended local/browser setup and enforced module boundaries, see [hybrid rendering](/docs/api/full-stack/hybrid-rendering). Its examples link to the production-build fixtures used by this integration.
...
Astro selects the `/api/[...api]` host route and passes its native `APIContext`; @hulla/api then matches the complete request
...
and `route` is @hulla/api matched contract operation rather than Astro's catch-all file route. Define `App.Locals` in
...
that contains the component. Accordingly, @hulla/api context sees that internal native request while its `route` metadata
...
Astro owns the island protocol, encrypted props, fallback, request, rendering, and swap. The @hulla/api transport only
...
The hook may return a replacement `Response` or `undefined` to retain @hulla/api protocol-safe default. Large APIs can mount
