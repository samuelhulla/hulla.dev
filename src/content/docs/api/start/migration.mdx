...
description: 'Move from procedure and builder APIs to contract selections, direct clients, exhaustive servers, and explicit transports.'
---

import DocsCodeDiff from '@/components/docs/DocsCodeDiff.astro'

Version 2.0 replaces callable procedure builders with an HTTP-first contract and explicit bindings. Move route shape into a shared contract, move behavior into an exhaustive server implementation, and construct clients directly from the contract node they need.

<DocsCodeDiff
  filename="src/api/users.ts"
  caption="The contract and handler are now separate values; transport choice is no longer embedded in procedure construction."
  diff={`-import { createApi } from '@hulla/api'
-
-const api = createApi()
-export const users = api.router('users').define(({ route }) => ({
-  byId: route('GET', '/:id')
-    .input(z.string())
-    .output(userSchema)
-    .handler(({ input }) => repository.find(input)),
-}))
...
+  byId: ({ params, response }) => response(200, repository.find(params.id)),
+})`}
/>

The rest of this guide covers later 2.0 builder-to-selection changes and runtime details. If you are migrating directly from the procedure API, treat the [contract](/docs/api/core/contracts), [server](/docs/api/core/servers), and [client](/docs/api/core/clients) guides as the authoritative replacement rather than translating builder methods one by one.
