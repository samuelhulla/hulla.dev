...
description: 'Compare routing, parsing, streaming, context, and lifecycle ownership across every supported host adapter.'
---
...
[`scripts/adapter-conformance.ts`](https://github.com/hulladev/api/tree/main/scripts/adapter-conformance.ts) registers the same contract and assertions for each
...
test, including failures. [`scripts/conformance-http.ts`](https://github.com/hulladev/api/tree/main/scripts/conformance-http.ts) provides real ephemeral TCP
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| Fetch          | [transport-conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/message-port/tests/transport-conformance.test.ts) | Uses native Request/Response without a network socket                   |
| In-process     | [transport-conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/message-port/tests/transport-conformance.test.ts) | No malformed JSON text scenario: input is already an application value  |
| MessagePort    | [transport-conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/message-port/tests/transport-conformance.test.ts) | No malformed JSON text scenario: messages use structured clone          |
| Node HTTP      | [conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/node/tests/conformance.test.ts)                             | Real TCP server                                                         |
| Express        | [conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/express/tests/conformance.test.ts)                          | Real TCP server; multipart parser is application-owned                  |
| Fastify        | [conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/fastify/tests/conformance.test.ts)                          | Real TCP server; multipart requires a host plugin                       |
| Hono           | [conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/hono/tests/conformance.test.ts)                             | Native router and Fetch handler                                         |
| H3             | [conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/h3/tests/conformance.test.ts)                               | Safe native parameter decoding preserves `%2F` instead of producing `/` |
| Elysia         | [conformance.test.ts](https://github.com/hulladev/api/tree/main/packages/elysia/tests/conformance.test.ts)                           | Native router and Fetch handler                                         |
...
| -------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Cloudflare Workers / Pages | Fetch representations and Web streams                                                     | Worker `fetch` / Pages file routing; native bindings and execution context              | [Tests](https://github.com/hulladev/api/tree/main/packages/cloudflare/tests)                                           |
| AWS Lambda                 | Payload v2 base64/text input; response streams are collected into a buffered proxy result | HTTP API v2 / Function URL event; platform owns request limits and invocation lifetime  | [Tests](https://github.com/hulladev/api/tree/main/packages/aws-lambda/tests)                                           |
| Azure Functions            | Native HTTP request/result mapping; byte streams passed as async iterables                | Functions v4 HTTP trigger; host owns buffering and lifetime                             | [Tests](https://github.com/hulladev/api/tree/main/packages/azure-functions/tests)                                      |
| Google Cloud functions     | Buffered `rawBody` input when provided; shared Node streaming response writer             | Functions Framework request/response context and platform request limits                | [Tests](https://github.com/hulladev/api/tree/main/packages/google-cloud-functions/tests)                               |
| Netlify Functions          | Fetch representations and Web streams                                                     | Web-native synchronous Functions context and platform deployment                        | [Tests](https://github.com/hulladev/api/tree/main/packages/netlify-functions/tests)                                    |
| Next.js                    | Fetch representations                                                                     | App Router; rejects `QUERY`; request context and Data Cache belong to Next              | [Production fixture](https://github.com/hulladev/api/tree/main/packages/next/tests/next-build.test.ts)                 |
| TanStack Start             | Fetch representations                                                                     | Wildcard server routes; rejects `QUERY`; loader and hydration lifecycle belong to Start | [Production fixture](https://github.com/hulladev/api/tree/main/packages/tanstack-start/tests/start-build.test.ts)      |
| React Router               | Fetch representations                                                                     | Resource routes; rejects `QUERY`; HEAD dispatches GET and suppresses its body           | [Production fixture](https://github.com/hulladev/api/tree/main/packages/react-router/tests/react-router-build.test.ts) |
| SolidStart                 | Fetch representations                                                                     | API routes; rejects `QUERY`; native event and GET/HEAD mapping                          | [Production fixture](https://github.com/hulladev/api/tree/main/packages/solid-start/tests/solid-start-build.test.ts)   |
| SvelteKit                  | Fetch endpoints and colocated remote-function transport                                   | Rejects `QUERY`; native RequestEvent and remote-function lifecycle                      | [Production fixture](https://github.com/hulladev/api/tree/main/packages/sveltekit/tests/sveltekit-build.test.ts)       |
| Nuxt                       | Nitro HTTP and request-aware client transport                                             | Rejects `QUERY`; native H3 event, HEAD mapping and SSR request state                    | [Production fixture](https://github.com/hulladev/api/tree/main/packages/nuxt/tests/nuxt-build.test.ts)                 |
| Astro                      | Fetch endpoints and explicit-context in-process calls                                     | Native APIContext, HEAD mapping and server-island lifecycle                             | [Production fixture](https://github.com/hulladev/api/tree/main/packages/astro/tests/astro-build.test.ts)               |
...
[packed consumer fixtures](https://github.com/hulladev/api/tree/main/scripts/package-consumers). These exercise adapter mounting/native context or transport
...
[`scripts/check-packages.ts`](https://github.com/hulladev/api/tree/main/scripts/check-packages.ts) owns the host-major profiles and emits the CI matrix with
...
See [WebSocket](/docs/api/transports/websocket), [NestJS](/docs/api/servers/nestjs) and [runtime hosts](/docs/api/runtimes/runtime-hosts) for ownership and setup.
...
See [Koa](/docs/api/servers/koa) for ownership and commands.
...
MessagePort implementation. See [desktop bridges](/docs/api/transports/desktop-bridges) for native setup and lifecycle responsibilities.
