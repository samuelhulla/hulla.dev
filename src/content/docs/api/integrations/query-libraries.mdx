...
description: 'Derive cache keys and executable query or mutation options from an existing typed client.'
---

Client integrations are explicit parallel views over the client described in
[client authoring](/docs/api/core/clients). They do not register with `createClient()`, mutate route calls, add `$`
