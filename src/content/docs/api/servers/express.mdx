---
title: 'Express'
description: 'Register compiled contract routes in Express while retaining Express request and response context.'
---
...
function listen(port: number): void {
  ...
  server.once('error', (error) => {
    if ((error as NodeJS.ErrnoException).code === 'EADDRINUSE' && port !== 0) {
      console.warn(
        `Port ${port} is occupied; selecting an available port instead`
      )
      listen(0)
      return
    }
...
application; see [client authoring](/docs/api/core/clients) and [client integrations](/docs/api/integrations/query-libraries).
