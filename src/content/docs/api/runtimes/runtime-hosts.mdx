...
description: 'Choose the Fetch or Node boundary that fits Bun, Deno, and Vercel runtime deployments.'
---

These hosts use the existing `fetchAdapter()` from `@hulla/api/fetch`; no additional adapter package is needed.
The runnable fixtures are in [`examples/runtime-hosts`](https://github.com/hulladev/api/tree/main/examples/runtime-hosts).
